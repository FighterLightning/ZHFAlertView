...
class PopCheckboxButtonView: PopSmallChangeBigFatherView {
    var delegate:PopCheckboxButtonViewDelegate?
    let titleHeight: CGFloat = 75
    var contentArr : [String] = [String]()
    lazy var contentMarr: NSMutableArray = NSMutableArray()
    ...
    @objc func btnClick(btn:UIButton){
        ...
        if btn.isSelected == true {
             ...
             contentMarr.add((btn.titleLabel?.text)!)
        }
        else{
             btn.layer.borderColor = ZHFColor.zhf88_contentTextColor.cgColor
             contentMarr.remove((btn.titleLabel?.text)!)
        }
    }
    @objc func trueBtnClick(btn:UIButton){
        self.delegate?.selectMessage(contentMarr: (contentMarr))
    }
}
